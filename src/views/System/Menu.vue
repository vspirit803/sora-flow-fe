  ...
  <div>
    ...
    <v-list expand>
      ...
      <template v-for="each of menus">
        <v-list-item v-if="each.type === 'item'" :key="each.id" link>
          <v-list-item-icon v-if="each.icon">
            <v-icon>{{ each.icon }}</v-icon>
          </v-list-item-icon>
          <v-list-item-title>{{ each.name }}</v-list-item-title>
          <v-list-item-action class="flex-row">
            <v-btn icon text color="primary" title="编辑菜单项" @click.stop="updateMenu(each)">
              <v-icon>mdi-file-document-edit</v-icon>
            </v-btn>
            <Confirm v-slot="{ on, attrs }" :message="`确认删除菜单[${each.name}]吗`" @confirm="deleteMenu(each)">
              <v-btn icon text color="error" title="删除菜单项" v-bind="attrs" v-on="on">
        ...
        <v-list-group v-else :key="each.id">
          <template v-slot:activator>
            ...
            <v-list-item-title>{{ each.name }}</v-list-item-title>
            <v-list-item-action class="flex-row">
              <v-btn icon text color="primary" title="添加菜单项" @click.stop="createMenu(each, 'item')">
                <v-icon>mdi-file-plus</v-icon>
              </v-btn>
              <v-btn icon text color="primary" title="添加子菜单" @click.stop="createMenu(each, 'directory')">
                <v-icon>mdi-folder-plus</v-icon>
              </v-btn>
              <v-btn icon text color="primary" title="编辑菜单" @click.stop="updateMenu(each)">
                <v-icon>mdi-folder-edit</v-icon>
              </v-btn>
              <Confirm v-slot="{ on, attrs }" :message="`确认删除菜单[${each.name}]吗`" @confirm="deleteMenu(each)">
                <v-btn icon text color="error" title="删除菜单" v-bind="attrs" v-on="on">
          ...
          <template v-for="eachSub of each.children">
            <v-list-item v-if="eachSub.type === 'item'" :key="eachSub.id" link>
              <v-list-item-icon v-if="eachSub.icon">
                <v-icon>{{ eachSub.icon }}</v-icon>
              </v-list-item-icon>
              <v-list-item-title>{{ eachSub.name }}</v-list-item-title>
              <v-list-item-action class="flex-row">
                <v-btn icon text color="primary" title="编辑菜单项" @click.stop="updateMenu(eachSub)">
                  <v-icon>mdi-file-document-edit</v-icon>
                </v-btn>
                <Confirm
                  v-slot="{ on, attrs }"
            ...
            <v-list-group sub-group v-else :key="eachSub.id">
              <template v-slot:activator>
                ...
                <v-list-item-title>{{ eachSub.name }}</v-list-item-title>
                <v-list-item-action class="flex-row">
                  <v-btn icon text color="primary" title="添加菜单项" @click.stop="createMenu(eachSub, 'item')">
                    <v-icon>mdi-file-plus</v-icon>
                  </v-btn>
                  <v-btn icon text color="primary" title="编辑菜单" @click.stop="updateMenu(eachSub)">
                    <v-icon>mdi-folder-edit</v-icon>
                  </v-btn>
                  <Confirm
                    v-slot="{ on, attrs }"
              ...
              <!--三级菜单-->
              <v-list-item v-for="eachSubSub of eachSub.children" :key="eachSubSub.id" link>
                <v-list-item-icon v-if="eachSubSub.icon">
                  <v-icon>{{ eachSubSub.icon }}</v-icon>
                </v-list-item-icon>
                <v-list-item-title>{{ eachSubSub.name }}</v-list-item-title>
                <v-list-item-action class="flex-row">
                  <v-btn icon text color="primary" title="编辑菜单项" @click.stop="updateMenu(eachSubSub)">
                    <v-icon>mdi-file-document-edit</v-icon>
                  </v-btn>
                  <Confirm
                    v-slot="{ on, attrs }"
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12">
                <v-text-field label="名称" required v-model="menuModel.name"></v-text-field>
              </v-col>
              <v-col cols="12">
                <v-text-field label="icon" v-model="menuModel.icon"></v-text-field>
              </v-col>
              <v-col v-if="menuModel.type === 'item'" cols="12">
                <v-text-field label="url" required v-model="menuModel.url"></v-text-field>
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
